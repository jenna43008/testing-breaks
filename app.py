...
- ESTABLISHED AUTHENTICATED SITE DEFENSE: Generalized e-commerce phishing kit
  defense to cover any established (90+ day) domain with strong email auth
  (enterprise MX + DKIM, or DMARC reject/quarantine + DKIM). credential_form and
  form_posts_external are suppressed on these domains — they're normal business
  features (customer logins, payment/banking processors). Covers financial
  institutions (credit unions, banks), SaaS, and any enterprise site with a login.
- TRANSFER LOCK FULLY-AUTHENTICATED DOMAIN REDUCTION: On domains with enterprise
  MX + DKIM + SPF strict (-all), transfer_lock_with_risk scores 0 (informational)
  when the only risk signals are "soft" indicators (vulnerable plugins, PTR mismatch).
  Domains investing in full email authentication are more likely to have
  administrative lock changes than post-compromise lockdowns.
- ZERO EMAIL AUTH FLOOR: Domains with NO SPF, NO DKIM, AND NO DMARC are
  guaranteed denial (score floor = threshold + 5). Gmail and Yahoo reject emails
  from domains with zero authentication — approving them has no business value.
